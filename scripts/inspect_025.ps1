...
$matches = $allProducts | Where-Object { $_.node.title -match "025" -or $_.node.handle -match "025" }
...
foreach ($m in $matches) {
    ...
    $idx = 0
    foreach ($med in $node.media.edges) {
        Write-Host "  [$idx] MediaID=$($med.node.id), Alt=$($med.node.alt)"
        Write-Host "      URL: $($med.node.image.url)"
        $idx++
    }
